...
if (requireNamespace("rnaturalearth", quietly = TRUE)) {
  madagascar <- rnaturalearth::ne_countries(scale = 50, country = "Madagascar",
                                            returnclass = "sf") |>
    st_make_valid()
} else {
...
zahamena_pa       <- wdpa |> filter(grepl("Zahamena", NAME) & !grepl("Ankeniheny", NAME))
...
maro_fkt$dist_km <- round(
  as.numeric(st_distance(maro_fkt, st_union(ankarafantsika_pa))) / 1000, 1
)

ala_fkt <- tibble(
  fokontany = c("Avaradrano", "Feramanga Atsimo", "Mangabe",
                "Analamiranga", "Maritampona", "Ambohidrony", "Ambatomanga"),
  lon = c(48.469958, 48.373383, 48.405244, 48.200017,
          48.207639, 48.208611, 48.200647),
  lat = c(-17.805414, -17.844153, -17.879586, -17.572375,
          -17.603500, -17.729175, -17.741178)
) |>
  st_as_sf(coords = c("lon", "lat"), crs = 4326)
ala_fkt$dist_php <- round(
  as.numeric(st_distance(ala_fkt, st_union(alaotra_pa))) / 1000, 1
)
# CAZ/Zahamena not in dynamic WDPA — set dist_caz = Inf so nearest_pa defaults
# to "Lac Alaotra PHP". If Vahatra data becomes available, recompute.
if (nrow(caz_pa) > 0) {
  ala_fkt$dist_caz <- round(
    as.numeric(st_distance(ala_fkt, st_union(caz_pa))) / 1000, 1
  )
} else {
...
obs_locations <- tibble(
  ...
  mutate(role = factor(role, levels = c(
