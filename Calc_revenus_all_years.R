...
process_revel <- function(year, path = "data/ROS_MDG_microdata/") {
  ...
  # Vente d’animaux : bovins vs autres
  vente_ani <- ele %>%
    mutate(
  ...
  # Conso zébu (pour filtrer dans la vente viande ensuite)
  conso_zebu <- ele %>%
    filter(ele_lig %in% 1:3, !is.na(ele_e), ele_e != 0) %>%
    group_by(j5, year) %>%
    summarise(conso_zebu = 1, .groups = "drop")
  
  # Vente produits animaux (hors zébu, viande séparée)
  ...
  vente_prodel <- pe %>%
    ...
    mutate(
      vente_lait = if_else(pe_lig == 1, pe_e * pe_c, 0),
      vente_oeuf = if_else(pe_lig == 2, pe_e * pe_c, 0),
      vente_beef = if_else(pe_lig == 3, pe_e * pe_c, 0),
      vente_meat = if_else(pe_lig %in% c(4, 6, 7), pe_e * pe_c, 0)
    ) %>%
    group_by(j5, year) %>%
    summarise(
...
revel_all <- map_dfr(years, ~process_revel(.x, path = "data/ROS_MDG_microdata/"))
