...
process_revppal <- function(path = "data/ROS_MDG_microdata/", year) {
  ...
  if(year > 2011) {
    ...
    read_dta(paste0(path, year, "/res_m_a.dta")) %>%
      mutate(a3c = replace_na(a3c, 0)) %>%
      mutate(revppal = a3c * 52 * 1000) %>%
      group_by(j5, year) %>%
...
process_prod_riz_val <- function(path = "data/ROS_MDG_microdata/", year) {
...
## rente_riz ------------------
process_rente_riz <- function(path = "data/ROS_MDG_microdata/", year) {
  stopifnot(year %in% 1995:2015)
...
process_charge_riz <- function(path = "data/ROS_MDG_microdata/", year) {
  ...
  coutmori <- read_dta(paste0(path, year, file_mo)) %>%
    mutate(across(
      c(mo11b, mo11c, mo11d, mo11e, mo51a, mo51b, mo12, mo51c, mo23),
      ~ replace_na(.x, 0)
    )) %>%
    mutate(
      salarie = (mo11b * mo11d) + (mo11b * mo51a) +
        (mo11c * mo11e) + (mo11c * mo51b),
      tache = mo12 + mo51c,
      entraide = mo23,
  ...
  # Agrégation finale
  df <- reduce(
    list(coutmori, coutint, coutmetloc),
...
test <- map_dfr(2009:2015, function(y) {
  df <- process_charge_riz(path = "data/ROS_MDG_microdata/", year = y)
  df %>%
    summarise(
      year = y,
      n = n(),
      moyenne = mean(charge_riz, na.rm = TRUE),
      mediane = median(charge_riz, na.rm = TRUE),
      min = min(charge_riz, na.rm = TRUE),
      max = max(charge_riz, na.rm = TRUE)
    )
})
