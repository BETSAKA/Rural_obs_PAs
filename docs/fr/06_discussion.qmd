...
Extending the analysis to Farafangana, Toliara North, and Fénérive East yields near-zero, statistically insignificant ATTs in all three cases. These sites face protected areas that are either multipurpose in governance, small in footprint, or both. The 2025 resurvey at Marovoay confirms via within-observatory DiD that the east-west income gap persists twenty-two years after the park extension, though its interpretation remains subject to the regional-decline caveat discussed above.
...
The inter-observatory CS-DiD presented in [Appendix C](A3_robustness.qmd) already implements staggered Callaway--Sant'Anna estimation at the household level with `panel = FALSE`, pooling the Marovoay (g = 2003) and Alaotra (g = 2008) cohorts against the six clean donor observatories. Results confirm the qualitative pattern from gsynth: a negative Marovoay cohort ATT and a null Alaotra cohort ATT.

When the 2026 survey round becomes available, the estimator can be extended with a fifth cohort (Fénérive East, once its post-treatment outcomes are observed). A Sun--Abraham [-@SunAbraham2021] interaction-weighted estimator via `fixest::sunab()` can provide a transparent event-study summary alongside the CS estimator. If gsynth and pooled DiD agree across the extended set of sites, the evidence is more credible regardless of which identifying assumption one prefers. If the 2026 wave covers Fénérive East, the small footprint of the Réserve de Tampolo allows a within-observatory distance design contrasting near-PA and far-PA sites, adding a third independent geographic context alongside Marovoay and Alaotra.

The current gsynth results should be read as a first-pass analysis that will benefit from household-level estimation exploiting the full depth of the data once survey depth and coverage expand.